...
async function calcSthAptApr() {
    ...
    console.log(`1 sthAPT = ${rate} thAPT at ${new Date(Number(info.ledger_timestamp) / 1000).toLocaleString('en-US')}`);
    console.log(`1 sthAPT = ${pastRate} thAPT at ${new Date(Number(pastInfo.block_timestamp) / 1000).toLocaleString('en-US')}`);
    return (
      ((rate - pastRate) /
        pastRate /
...
async function getSthAptThAptExchangeRateSynced(ledgerVersion?: string) {
    const result = await client.view({
        function: "0ebb91f1b0d422aa0820de017e9b54a796167dc500ad4324f389e55856c63aaa::staking::thAPT_sthAPT_exchange_rate_synced",
        type_arguments: [],
        arguments: [],
        }, ledgerVersion);
